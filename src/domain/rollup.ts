...
import type { ChildRef } from "./types.ts";
 ...
 */
export function deriveStatus(children: ChildRef[]): Status {
  if (children.every((c) => c.status === "done")) return "done";
  if (children.some((c) => c.status === "in-progress")) return "in-progress";
  if (children.some((c) => c.status === "backlog" || c.status === null)) return "backlog";
  return "ready";
}
 ...
 * ゲートしない。
 */
export function gatePlanPromotion(current: Status | null, derived: Status): Status {
  return derived === "ready" && current === "backlog" ? "backlog" : derived;
}

/** 子の SP 合計。SP を持つ子が 1 件もなければ null(見積もり を書かない) */
export function deriveSp(children: ChildRef[]): number | null {
  const sps = children.map((c) => c.sp).filter((sp): sp is number => sp !== null);
