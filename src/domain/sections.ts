...
const startMarkerRe = (prefix: string): RegExp =>
  new RegExp(`^<!--\\s*${escapeRegExp(prefix)}:section:(.+?):start\\s*-->\\s*$`);

const endMarkerRe = (prefix: string, name: string): RegExp =>
  new RegExp(`^<!--\\s*${escapeRegExp(prefix)}:section:${escapeRegExp(name)}:end\\s*-->\\s*$`);
...
function consumeOutside(st: ParseState, line: string): void {
  const m = line.match(st.startRe);
  if (m?.[1] !== undefined) {
    st.current = m[1];
    return;
  }
  if (FENCE_RE.test(line)) st.inFence = !st.inFence;
}

function consumeLine(st: ParseState, line: string): void {
  ...
  if (FENCE_RE.test(line)) st.inFence = !st.inFence;
  if (st.current !== null) st.buf.push(line);
}
 ...
 */
export function parseSections(body: string, prefix: string): Record<string, string> {
  const st: ParseState = {
    prefix,
...
export function renderBody(sections: Record<string, string>, prefix: string): string {
