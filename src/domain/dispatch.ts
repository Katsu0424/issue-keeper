...
import { deriveStatus } from "./rollup.ts";
import type { Kind, Status } from "./schema.ts";
import { kindOf, type Snapshot, statusOf } from "./types.ts";
...
function dispatchTask(s: Snapshot, status: Status): NextStep {
  if (status === "ready") {
    return {
      action: "start-task",
...
function dispatchContainerWithoutReadyChild(s: Snapshot): NextStep {
  ...
  if (inProgressKids.length > 0) {
    parts.push(
      `In Progress の子(${inProgressKids.map((c) => `#${c.number}`).join(", ")})の作業を続行してください`,
    );
  }
  if (parts.length === 0 && deriveStatus(s.children) === "done") {
    parts.push(
      "すべての子が完了していますがロールアップが未反映です。任意の子孫への CLI 実行で追いつきます",
    );
  }
  return {
    action: "error-state",
...
function dispatchContainer(s: Snapshot): NextStep {
...
export function dispatch(s: Snapshot): NextStep {
  const wu = classify(s);
  if (wu === "Malformed") return dispatchMalformed(s);
  ...
  if (wu === "Task") return dispatchTask(s, status as Status);
