...
import { planAdr, planBug, planEpic, planFeature } from "../usecase/plan.ts";
import { type CliContext, out, parseIssueNumber, parseSpArg, readValue } from "./shared.ts";
...
export function registerPlanCommands(program: Command, ctx: () => CliContext): void {
  program
    ...
    .action(async (n: number, opts: { requirements: string; acceptance: string; sp: number }) => {
      const { cfg, repo } = ctx();
      out(
        await planFeature(repo, cfg, n, {
  ...
  program
    ...
    .requiredOption("--sp <N>", "ストーリーポイント", parseSpArg)
    .action(async (n: number, opts: { report: string; sp: number }) => {
      const { cfg, repo } = ctx();
  ...
  program
    ...
    .requiredOption("--sp <N>", "ストーリーポイント", parseSpArg)
    .action(async (n: number, opts: { decision: string; alternatives?: string; sp: number }) => {
      const { cfg, repo } = ctx();
      out(
        await planAdr(repo, cfg, n, {
  ...
  program
    ...
    .action(async (n: number, opts: { scope: string }) => {
      const { cfg, repo } = ctx();
